...
const RegisterPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(error);
      let finalErrorMsg = 'Registration failed. Please check inputs.';
      if (error.code === 'ERR_NETWORK' || error.message === 'Network Error') {
        ...
        finalErrorMsg = error.response.data.errors.map((e: any) => e.defaultMessage || e.msg).join(', ');
        ...
        <section className="flex flex-col gap-8 text-left">
          ...
          <div>
            <div className="flex items-center gap-3 mb-6 cursor-pointer" onClick={() => navigate('/')}>
              ...
              <img src="/name.png" alt="SyncStream" className="h-9 w-40 object-contain shrink-0" />
            </div>
